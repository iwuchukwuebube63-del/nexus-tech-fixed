...
import {
  ...
  toggleCloudMinerLock,
} from "./db";
import { z } from "zod";
import { hashPassword, verifyPassword, generateUserId, generateReferralCode } from "./auth";
import { eq, and } from "drizzle-orm";
import { sdk } from "./_core/sdk";
import { users, transactions as transactionsTable, giftCodes, notifications, userNotificationReads } from "../drizzle/schema";
...
export const appRouter = router({
  ...
  wallet: router({
    ...
    deposit: protectedProcedure
      .input(z.object({ amount: z.number().min(3000).max(10000000), receiptBase64: z.string().optional(), paystackRef: z.string().optional() }))
      .mutation(async ({ input, ctx }) => {
        const user = await getUserById(ctx.user.id);
