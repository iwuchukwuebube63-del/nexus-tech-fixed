...
export default function Deposit() {
  ...
  const paystackEnabled = settingsQuery.data?.paystackEnabled !== false;
  const isPaystack = paystackEnabled && parsed > 0 && parsed <= 20000;
  ...
  const handleContinue = async () => {
    ...
    if (isPaystack) {
      ...
      try {
        ...
        if (err.message === "PAYSTACK_DISABLED") {
          // Admin disabled Paystack — fall through to manual transfer
          setStep("manual");
          setSubmitting(false);
        } else {
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        {step === "manual" && (
          <>
            ...
            <button onClick={handleManualSubmit} disabled={submitting || timerExpired || !receiptBase64}
              ...
              {submitting ? "Submitting..." : !receiptBase64 ? "Upload Receipt to Continue" : "I Have Transferred — Submit"}
            </button>
          </>
        )}
      </main>
